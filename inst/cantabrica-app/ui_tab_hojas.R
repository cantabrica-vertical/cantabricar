        ...
        fluidPage(
            ...
            fluidRow(
                box(
                    ...
                    status = "success",
                    width = 4,
                    selectInput(inputId = "database_hojas_id", label = "ID", choices = sort(pull(filter(d$sembradas, !(id %in% d$cosechadas$id), !(id %in% d$hojas$id)), id)), selected = NULL, multiple = TRUE),
